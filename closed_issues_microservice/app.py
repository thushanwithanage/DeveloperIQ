...
@app.get("/")
async def root():
    return {"response": "Github closed issues route"}
...
async def created_issues(user: str):
    ...
    return json.loads(str(closed_issue_count_by_user))
